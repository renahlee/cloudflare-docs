...
{props.productName} customers can configure Magic health check alerts to receive email, webhook, and PagerDuty notifications when a given error budget for unsuccessful <GlossaryTooltip term="tunnel health-check">health checks</GlossaryTooltip> are exhausted and the <a href={props.magicTunnelHealthCheckCalculation}>service-level objective (SLO)</a> is violated.

Magic health check alerts monitor the health of connectors with active traffic. Customers can define an SLO threshold from which an error budget is derived. If the number of unsuccessful health checks for the Magic connector(s) exceeds the allotted error budget, an alert fires.

## Alert data

If a Magic health check alert is fired, customers can expect the following data in the email, webhook, and PagerDuty notification:
...
## Set up Magic health check alerts
...
4. Select **Magic Health Check Alert** > **Select** to add a notification. You can add alerts by tunnel or by data center (beta).
...
11. Select the connectors for the data center you want to receive alerts for, and select **Next**.
...
Send a [`POST` request](/api/resources/alerting/subresources/policies/methods/create/) to create a {props.productName} Magic health check alert. You can set Magic health check alerts with any SLO value between `0` and `99.99`.
